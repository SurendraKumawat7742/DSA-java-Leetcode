class TaskManager {
    ...
    private Map<Integer, Integer> taskPrio;
    ...
    public void add(int userId, int taskId, int priority) {
        ...
        taskPrio.put(taskId, priority);
        taskOwner.put(taskId, userId);
    }
    
    public void edit(int taskId, int newPriority) {
        pq.add(new int[]{newPriority, taskId});
        taskPrio.put(taskId, newPriority);
    }
    
    public void rmv(int taskId) {
        taskPrio.put(taskId, -1);
    }
    ...
    public int execTop() {
        while(!pq.isEmpty()){
            int[] t = pq.poll();
            int p = t[0], id = t[1];
            if(taskPrio.getOrDefault(id, -2) == p){
                taskPrio.put(id, -1);
